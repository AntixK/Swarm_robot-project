...
ALPHA:
  ...
  {
    communicate();
    if ((trgt < min(B_status.target, C_status.target)) || flag2 ==1 )
    {
BETA:
      beta();
      goto END;
...
void beta()
{
  ...
  set_beta();
  int turn_angle;
  if ( B_status.target > C_status.target)
  {
    turn_angle =  B_status.heading;

  }
  else if ( B_status.target < C_status.target)
  {
...
void communicate()   // better way
{
   for (int retry = 0; retry < 5; retry += 1)
    {
      send_data('A', heading, trgt, Dist, State);
      delay(10);
      network.update();
    }
