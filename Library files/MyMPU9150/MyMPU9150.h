...
private: 
  ...
  float head_hist[9],accX_hist[9], accY_hist[9] ;
...
public:
  ...
  void setup_hist()
  {
    ...
    { 
      ...
      get_Heading(&head);
      head_hist[i] = head;
      accX_hist[i] = (float)(MPU9150_readSensor(MPU9150_ACCEL_XOUT_L,MPU9150_ACCEL_XOUT_H))/ACC_SENS;
      accY_hist[i] = (float)(MPU9150_readSensor(MPU9150_ACCEL_YOUT_L,MPU9150_ACCEL_YOUT_H))/ACC_SENS;
    }
  ...
  float get_SGAfilt(float val,float hist[5])
  {
    /* SGA Coefficients*/
    ...
    {
      sum+= hist[i+SGA_MID]*sga_coefficients[i+SGA_MID];
    }
  ...
  float get_filt(float val,uint8_t flag)
  {
    switch(flag)
    {
    case 1:
      return(get_SGAfilt(val,head_hist));
      break;

    case 2:
      return(get_SGAfilt(val, accX_hist));
      break;

    case 3:
      return(get_SGAfilt(val, accY_hist));
      break;
