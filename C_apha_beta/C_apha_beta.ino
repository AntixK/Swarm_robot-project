...
ALPHA:
  ...
  {
    communicate();
    if ((trgt < min(A_status.target, B_sttatus.target)) || flag2 ==1 )
    {
BETA:
      beta();
      goto END;
...
void beta()
{
  ...
  set_beta();
  int turn_angle;
  if ( A_status.target > B_sttatus.target)
  {
    turn_angle =  A_status.heading;

  }
  else if ( A_status.target < B_sttatus.target)
  {
    turn_angle =  B_sttatus.heading;
